    ...
    if (isset($_POST['title']) && !isset($_POST['add_category_name'])) {
        ...
        $tags = isset($_POST['tags']) ? explode(',', $_POST['tags']) : [];
            ...
            if (empty($title) || empty($content) || empty($category_id) || empty($visibility)) {
                 ...
                 // Update the article
                    $stmt = $conn->prepare("UPDATE knowledge_base_articles SET title = :title, content = :content, category_id = :category_id, visibility = :visibility, access_level = :access_level WHERE id = :id");
                    $stmt->bindParam(':id', $article_id);
                   ...
                   $stmt->bindParam(':visibility', $visibility);
                     ...
                     if ($stmt->execute()) {
                         // delete existing tags.
                            $stmt = $conn->prepare("DELETE FROM knowledge_base_article_tags WHERE article_id = :article_id");
...
$tags_string = implode(', ',$assigned_tags);
                  ...
                  <option value="all" <?php if($article['visibility'] == 'all') echo 'selected'; ?>>All Employees</option>
                   <option value="team" <?php if($article['visibility'] == 'team') echo 'selected'; ?>>Specific Teams</option>
                     <option value="admin" <?php if($article['visibility'] == 'admin') echo 'selected'; ?>>Admins Only</option>
                        <option value="draft" <?php if($article['visibility'] == 'draft') echo 'selected'; ?>>Draft</option>
                </select>
           </div>
           <div class="mb-4">
                  <label for="tags" class="block text-gray-700">Tags (comma-separated)</label>
                 <input type="text" name="tags" id="tags" class="w-full px-4 py-3 border rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-600" value="<?php echo htmlspecialchars($tags_string); ?>">
            </div>
            <div class="mb-4">
                <label for="access_level" class="block text-gray-700">Access Level</label>
                  <select name="access_level" id="access_level" class="w-full px-4 py-3 border rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-600 appearance-none">
                       <option value="public" <?php if($article['access_level'] == 'public') echo 'selected'; ?>>Public</option>
                       <option value="private" <?php if($article['access_level'] == 'private') echo 'selected'; ?>>Private</option>
                    </select>
               </div>
          <button type="submit" class="bg-blue-700 text-white px-6 py-3 rounded-xl hover:bg-blue-900 transition duration-300 shadow-md uppercase tracking-wide">Update Article</button>
      </form>
            <!-- Category Add Modal -->
           <div id="categoryModal" class="fixed z-10 inset-0 overflow-y-auto hidden">
            ...
            <!-- Success Toast -->
             ...
             <div class="mt-4">
                 <a href="<?php echo BASE_URL; ?>knowledge_base/view?id=<?php echo $article_id; ?>" class="bg-gray-600 text-white px-4 py-2 rounded-lg hover:bg-gray-700 transition duration-300 inline-block">Back to Article</a>
            </div>
    </div>
</div>
    <script>
