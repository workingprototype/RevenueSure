...
if (!isset($_GET['id'])) {
    header("Location: manage_knowledge_base_categories.php");
    exit();
}
               ...
               $stmt->bindParam(':parent_id', $parent_id);
                $stmt->bindParam(':id', $category_id);
                 if ($stmt->execute()) {
                    ...
                    header("Location: manage_knowledge_base_categories.php?success=true");
                      exit();
                 } else {
         ...
         <div class="bg-gray-100 border border-gray-400 p-6 rounded-lg">
          <form method="POST" action="">
            ...
            <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition duration-300 uppercase tracking-wide">Update Category</button>
               <div class="mt-4">
                 <a href="manage_knowledge_base_categories.php" class="bg-gray-600 text-white px-4 py-2 rounded-lg hover:bg-gray-700 transition duration-300 uppercase tracking-wide">Back to Categories</a>
                </div>
           </form>
        </div>
    </div>
