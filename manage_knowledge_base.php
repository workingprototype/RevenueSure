...
if(isset($_GET['search']) && !empty($_GET['search'])){
        ...
        $where_conditions[] = " (knowledge_base_articles.title LIKE :search OR knowledge_base_articles.content LIKE :search)";
...
<div class="container mx-auto p-6 fade-in">
    ...
    <div class="flex justify-between items-center mb-8">
       <a href="add_knowledge_base_article.php" class="bg-blue-700 text-white px-6 py-3 rounded-xl hover:bg-blue-900 transition duration-300 shadow-md uppercase tracking-wide">
        ...
        <div class="flex flex-wrap gap-2">
            <form method="GET" action="" class="flex gap-2 flex-wrap">
                <input type="text" name="search" id="search" placeholder="Search" class="px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600 flex-1" value="<?php echo isset($_GET['search']) ? $_GET['search'] : ''; ?>">
                 <select name="category_id" id="category_id" class="px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600">
              ...
              ?>
               <?php if ($hasFilters): ?>
                   <a href="manage_knowledge_base.php" class="bg-red-700 text-white px-4 py-2 rounded-lg hover:bg-gray-900 transition duration-300 shadow-md">
            ...
            <tbody>
                <?php if ($articles): ?>
                   <?php foreach ($articles as $article): ?>
                    ...
                    <?php else: ?>
                        <tr>
                         <td colspan="7" class="px-4 py-2 text-center text-gray-600">No articles found.</td>
                       </tr>
                  <?php endif; ?>
           </tbody>
       </table>
