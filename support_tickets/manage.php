...
<div class="container mx-auto p-6 fade-in">
  <h1 class="text-3xl font-bold text-gray-800 mb-6 uppercase tracking-wide border-b-2 border-gray-400 pb-2">
        ...
        <?php if ($tickets_by_status): ?>
          <ul class="list-inside pl-2">
            <?php foreach ($tickets_by_status as $status => $count): ?>
              <li class="text-sm">
                <span class="font-medium"><?php echo htmlspecialchars($status); ?></span>: 
                <span class="font-semibold"><?php echo htmlspecialchars($count); ?></span>
              </li>
            <?php endforeach; ?>
          </ul>
        <?php else: ?>
        ...
        <?php if ($tickets_by_priority): ?>
          <ul class="list-inside pl-2">
            <?php foreach ($tickets_by_priority as $priority => $count): ?>
              <li class="text-sm">
                <span class="font-medium"><?php echo htmlspecialchars($priority); ?></span>: 
                <span class="font-semibold"><?php echo htmlspecialchars($count); ?></span>
              </li>
            <?php endforeach; ?>
          </ul>
  ...
  <!-- Add Ticket Button & Filter Section -->
  <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 space-y-4 sm:space-y-0">
    <a href="<?php echo BASE_URL; ?>support_tickets/add" class="bg-blue-700 text-white px-6 py-3 rounded-xl hover:bg-blue-900 transition duration-300 shadow-md uppercase tracking-wide">
    ...
    <div class="flex flex-wrap items-center gap-4">
        ...
        <div>
          <select name="filter_assigned_to" id="filter_assigned_to" class="px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600 appearance-none">
            ...
            <?php foreach ($users as $user): ?>
              <option value="<?php echo $user['id']; ?>" <?php if(isset($_GET['filter_assigned_to']) && $_GET['filter_assigned_to'] == $user['id']) echo 'selected'; ?>>
      ...
      <?php if ($filters_applied): ?>
        <a href="<?php echo BASE_URL; ?>support_tickets/manage" class="bg-red-700 text-white px-4 py-2 rounded-lg hover:bg-gray-900 transition duration-300 shadow-md">
          CLEAR FILTER
        </a>
      <?php endif; ?>
    </div>
  ...
  <?php if ($filters_applied): ?>
    <div class="applied-filters mb-8 border border-gray-400 bg-gray-100 rounded-lg p-6">
      <h2 class="text-xl font-bold text-gray-800 mb-4">Applied Filters</h2>
  ...
  <div class="flex justify-end space-x-2 mb-4">
    <a href="?view=list" class="px-3 py-2 border rounded hover:bg-gray-200 <?php echo ($view == 'list') ? 'bg-blue-600 text-white' : 'text-gray-600'; ?>">List</a>
    <a href="?view=grid" class="px-3 py-2 border rounded hover:bg-gray-200 <?php echo ($view == 'grid') ? 'bg-blue-600 text-white' : 'text-gray-600'; ?>">Grid</a>
    <a href="?view=card" class="px-3 py-2 border rounded hover:bg-gray-200 <?php echo ($view == 'card') ? 'bg-blue-600 text-white' : 'text-gray-600'; ?>">Card</a>
    <a href="?view=kanban" class="px-3 py-2 border rounded hover:bg-gray-200 <?php echo ($view == 'kanban') ? 'bg-blue-600 text-white' : 'text-gray-600'; ?>">Kanban</a>
    <a href="?view=calendar" class="px-3 py-2 border rounded hover:bg-gray-200 <?php echo ($view == 'calendar') ? 'bg-blue-600 text-white' : 'text-gray-600'; ?>">Calendar</a>
  </div>
  ...
  <div class="mb-8 border border-gray-400 bg-gray-100 rounded-lg p-6">
    ...
    switch ($view) {
      case 'grid':
        include 'support_tickets/grid.php';
        break;
      case 'card':
        include 'support_tickets/card.php';
        break;
      case 'kanban':
        include 'support_tickets/kanban.php';
        break;
      case 'calendar':
        include 'support_tickets/calendar.php';
        break;
      default:
        include 'support_tickets/list.php';
        break;
    }
    ?>
  </div>

  <a href="<?php echo BASE_URL; ?>support_tickets/ticket_tasks" class="bg-gray-300 text-white px-4 py-2 rounded-lg hover:bg-gray-900 transition duration-300 shadow-md">
    <i class="fas fa-tasks mr-2"></i> Manage Ticket Related Tasks
  </a>
