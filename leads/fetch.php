...
// Helper function to add filter conditions and parameters
function addFilter(string $field, string $operator, string $value, array &$filters, array &$params, string $paramName): void
{
    if (!empty($value)) {
        $filters[] = "$field $operator $paramName";
        $params[$paramName] = $value;
    }
}
 //Use it only on the filters array.
// Add filters dynamically
if (!empty($search)) {
    $searchParam = "%" . $search . "%";
    $searchFilters = [
      "leads.name LIKE :searchName",
        "leads.email LIKE :searchEmail",
          "leads.phone LIKE :searchPhone"
    ];
    $filters[] = "(" . implode(" OR ", $searchFilters) . ")";
       $params[':searchName'] = $searchParam;
    $params[':searchEmail'] = $searchParam;
    $params[':searchPhone'] = $searchParam;
}
...
$query = "SELECT leads.*, employees.name as assigned_employee, customers.name as customer_name, customers.id as customer_id_new
          ...
          LEFT JOIN customers ON leads.customer_id = customers.id " . $whereClause . "
...
try {
    ...
    $total_leads = $stmtCount->fetch(PDO::FETCH_ASSOC)['total'];
                   ...
                   <div>
                          ...
                          <p class="text-gray-600 mb-2"><strong>Phone:</strong> <?php echo htmlspecialchars($lead['phone']); ?></p>
                            ...
                            <p class="text-gray-600 mb-2">
                                ...
                                <?php if($lead['customer_id_new']): ?>
                                    <a href="<?php echo BASE_URL; ?>customers/edit?id=<?php echo $lead['customer_id_new']; ?>" class="text-blue-600 hover:underline"><?php echo htmlspecialchars($lead['customer_name'] ? $lead['customer_name'] : $lead['customer_id_new']); ?></a>
                                <?php else: ?>
                                  N/A
                                <?php endif; ?>
                             </p>
                      ...
                      <div class="flex justify-between items-center">
                        <div class="flex space-x-2">
                            <a href="<?php echo BASE_URL; ?>leads/view?id=<?php echo $lead['id']; ?>" class="text-purple-600 hover:underline">View Lead</a>
                            <a href="<?php echo BASE_URL; ?>leads/edit?id=<?php echo $lead['id']; ?>" class="text-blue-600 hover:underline">Edit</a>
                        </div>
                           <div class="flex space-x-2">
                                <a href="<?php echo BASE_URL; ?>tasks/view?lead_id=<?php echo $lead['id']; ?>" class="text-green-600 hover:underline">View Tasks</a>
                                <a href="<?php echo BASE_URL; ?>leads/delete?id=<?php echo $lead['id']; ?>" class="text-red-600 hover:underline">Delete</a>
                           </div>
                      </div>
                  </div>
                <?php endforeach; ?>
