...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['selected_leads'])) {
    ...
    if (!empty($selected_leads)) {
        ...
        echo "<script>alert('Selected leads deleted successfully!'); window.location.href='leads/search';</script>";
    } else {
        echo "<script>alert('No leads selected for deletion.'); window.location.href='leads/search';</script>";
    }
} else {
