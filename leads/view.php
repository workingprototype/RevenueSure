...
<div class="bg-white p-6 rounded-lg shadow-md mb-8">
    <p><strong>Status:</strong> <?php echo htmlspecialchars($lead['status']); ?></p>
        <?php if ($lead['customer_id']): ?>
             <p><strong>Customer:</strong>  <a href="<?php echo BASE_URL; ?>customers/edit?id=<?php echo $lead['customer_id']; ?>" class="text-blue-600 hover:underline"> View Customer </a></p>
        <?php endif; ?>
</div>
...
<div class="bg-white p-6 rounded-lg shadow-md mb-8">
    ...
    <a href="<?php echo BASE_URL; ?>leads/track_behavior?lead_id=<?php echo $lead_id; ?>&action=website_visit" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition duration-300">Website Visit</a>
    <a href="<?php echo BASE_URL; ?>leads/track_behavior?lead_id=<?php echo $lead_id; ?>&action=email_open" class="bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 transition duration-300">Email Open</a>
    <a href="<?php echo BASE_URL; ?>leads/track_behavior?lead_id=<?php echo $lead_id; ?>&action=form_submission" class="bg-purple-600 text-white px-4 py-2 rounded-lg hover:bg-purple-700 transition duration-300">Form Submission</a>
</div>
...
<div id="file_form" class="bg-white p-6 rounded-lg shadow-md mb-8">
    <form method="POST" action="leads/upload_attachment" enctype="multipart/form-data">
    <?php echo csrfTokenInput(); ?>
...
<div id="notes_form" class="bg-white p-6 rounded-lg shadow-md mb-8 hidden">
    <form method="POST" action="leads/upload_attachment">
    <?php echo csrfTokenInput(); ?>
        <input type="hidden" name="lead_id" value="<?php echo $lead_id; ?>">
        ...
        if ($notes): ?>
            <?php foreach ($notes as $note): ?>
                <div class="bg-yellow-100 p-4 rounded-lg shadow-md mb-4">
                    ...
                    <a href="<?php echo BASE_URL; ?>delete_attachment.php?id=<?php echo $note['id']; ?>" class="text-red-600 hover:underline">Delete</a>
                </div>
            <?php endforeach; ?>
        <?php else: ?>
            ...
            <?php if ($attachments): ?>
                <?php foreach ($attachments as $attachment): ?>
                    <tr class="border-b">
                        ...
                        <td class="px-4 py-2">
                            <a href="<?php echo BASE_URL; ?><?php echo $attachment['file_path']; ?>" class="text-blue-600 hover:underline" download>Download</a>
                            <a href="<?php echo BASE_URL; ?>delete_attachment.php?id=<?php echo $attachment['id']; ?>" class="text-red-600 hover:underline ml-2">Delete</a>
                        </td>
                    </tr>
                <?php endforeach; ?>
            <?php else: ?>
