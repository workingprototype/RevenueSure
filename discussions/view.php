...
$stmt = $conn->prepare("SELECT discussion_messages.*,
...
// Fetch participants for users
$stmt = $conn->prepare("
    SELECT dp.*, u.username, u.profile_picture AS user_profile_picture
    FROM discussion_participants dp
    LEFT JOIN users u ON dp.participant_id = u.id
    WHERE dp.discussion_id = :discussion_id AND dp.participant_type = 'user'
");
$stmt->bindParam(':discussion_id', $discussion_id);
$stmt->execute();
$user_participants = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Fetch participants for employees
$stmt = $conn->prepare("
    SELECT dp.*, e.name, e.profile_picture AS employee_profile_picture
    FROM discussion_participants dp
    LEFT JOIN employees e ON dp.participant_id = e.id
    WHERE dp.discussion_id = :discussion_id AND dp.participant_type = 'employee'
");
$stmt->bindParam(':discussion_id', $discussion_id);
$stmt->execute();
$employee_participants = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Fetch participants for customers
$stmt = $conn->prepare("
    SELECT dp.*, c.name, c.profile_picture AS customer_profile_picture
    FROM discussion_participants dp
    LEFT JOIN customers c ON dp.participant_id = c.id
    WHERE dp.discussion_id = :discussion_id AND dp.participant_type = 'customer'
");
$stmt->bindParam(':discussion_id', $discussion_id);
$stmt->execute();
...
$stmt = $conn->prepare("SELECT COUNT(*) as unread_count FROM discussion_messages
LEFT JOIN discussion_participants ON discussion_messages.discussion_id = discussion_participants.discussion_id
WHERE discussion_messages.discussion_id = :discussion_id
AND discussion_messages.user_id != :user_id
AND discussion_messages.sent_at > (SELECT last_viewed FROM discussion_participants WHERE participant_id = :user_id2 AND discussion_participants.discussion_id = :discussion_id2)
OR (SELECT COUNT(*) FROM discussion_participants WHERE participant_id = :user_id3 AND discussion_participants.discussion_id = :discussion_id3 AND last_viewed IS NULL) > 0");
$stmt->bindParam(':discussion_id', $discussion_id);
$stmt->bindParam(':user_id', $user_id);
    ...
    <div class="bg-white border-2 border-gray-200 p-6 rounded-lg mb-8">
        <div class="flex justify-between items-center mb-4">
            ...
            <?php if ($participants): ?>
                <div class="flex items-center">
                    <strong class="mr-2 text-gray-700">Participants:</strong>
                    <div class="flex -space-x-2">
                        <?php foreach ($participants as $participant): ?>
                            <?php
                                if (!empty($participant['user_profile_picture'])) {
                                    $profilePic = BASE_URL . htmlspecialchars($participant['user_profile_picture']);
                                } elseif (!empty($participant['employee_profile_picture'])) {
                                    $profilePic = BASE_URL . htmlspecialchars($participant['employee_profile_picture']);
                                } elseif (!empty($participant['customer_profile_picture'])) {
                                    $profilePic = BASE_URL . htmlspecialchars($participant['customer_profile_picture']);
                                } else {
                                    $profilePic = BASE_URL . 'public/uploads/profile/default_profile.png';
                                }
                            ?>
                            <img src="<?php echo $profilePic; ?>" alt="Profile Picture" class="w-8 h-8 rounded-full border-2 border-white">
                        <?php endforeach; ?>
                    </div>
                </div>
            <?php else: ?>
        ...
        <div class="bg-white border border-gray-100 p-6 rounded-lg mb-8">
            ...
            <?php if ($messages): ?>
                <ul id="message-list">
                    <?php
                    // Recursive function to display messages as a threaded discussion
                    function displayMessages($messages, $parentId = null, $level = 0) {
                        global $conn, $discussion_id;
                        $maxIndentLevel = 5; // Maximum indentation level
                        foreach ($messages as $message) {
                            if ($message['parent_id'] == $parentId) {
                                // Determine profile picture based on user type
                                if ($message['user_type'] == 'user' && !empty($message['user_profile_picture'])) {
                                    $profilePic = '<img src="' . BASE_URL . htmlspecialchars($message['user_profile_picture']) . '" class="w-10 h-10 rounded-full object-cover" alt="Profile Picture">';
                                } elseif ($message['user_type'] == 'customer' && !empty($message['customer_profile_picture'])) {
                                    $profilePic = '<img src="' . BASE_URL . htmlspecialchars($message['customer_profile_picture']) . '" class="w-10 h-10 rounded-full object-cover" alt="Profile Picture">';
                                } elseif ($message['user_type'] == 'employee' && !empty($message['employee_profile_picture'])) {
                                    $profilePic = '<img src="' . BASE_URL . htmlspecialchars($message['employee_profile_picture']) . '" class="w-10 h-10 rounded-full object-cover" alt="Profile Picture">';
                                } else {
                                    $profilePic = '<img src="' . BASE_URL . 'images/default_profile.png" class="w-10 h-10 rounded-full object-cover" alt="Profile Picture">';
                                }

                                // Determine username
                                if ($message['user_type'] == 'user') {
                                    $username = htmlspecialchars($message['user_name']);
                                } elseif ($message['user_type'] == 'employee') {
                                    $username = htmlspecialchars($message['employee_name']);
                                } elseif ($message['user_type'] == 'customer') {
                                    $username = htmlspecialchars($message['customer_name']);
                                } else {
                                    $username = "Unknown";
                                }

                                // Use margin-left for thread indentation based on reply level
                                $indentLevel = min($level, $maxIndentLevel);
                                echo '<li class="my-4 border-l-4 border-gray-300 pl-4" data-comment-id="' . $message['id'] . '" style="margin-left: ' . ($indentLevel * 20) . 'px;">';
                                echo '<div class="flex space-x-4">';
                                    echo '<div class="flex-shrink-0">' . $profilePic . '</div>';
                                    echo '<div class="flex-1">';
                                        echo '<div class="bg-gray-50 p-4 rounded-lg shadow-sm">';
                                            echo '<div class="flex items-center justify-between">';
                                                echo '<h4 class="font-bold text-gray-800">' . $username . '</h4>';
                                                echo '<span class="text-sm text-gray-500">' . date('Y-m-d H:i', strtotime($message['sent_at'])) . '</span>';
                                            echo '</div>';
                                            echo '<p class="text-gray-800 mt-2">' . htmlspecialchars($message['message']) . '</p>';
                                        echo '</div>';
                                        echo '<div class="mt-2 flex space-x-2">';
                                            echo '<button onclick="replyToMessage(' . $message['id'] . ')" class="text-blue-600 hover:underline text-sm">Reply</button>';
                                            if ($_SESSION['user_id'] === $message['user_id']) {
                                                $timeLimit = strtotime($message['sent_at']) + (60 * 60); // 1 hour
                                                if (time() <= $timeLimit) {
                                                    echo '<a href="' . BASE_URL . 'discussions/edit?id=' . $message['id'] . '&discussion_id=' . $discussion_id . '" class="text-green-600 hover:underline text-sm">Edit</a>';
                                                }
                                                echo '<a href="' . BASE_URL . 'discussions/delete?id=' . $message['id'] . '&discussion_id=' . $discussion_id . '" class="text-red-600 hover:underline text-sm">Delete</a>';
                                            }
                                        echo '</div>';
                                        // Fetch and display attachments for the message
                                        ...
                                        if ($attachments) {
                                            ...
                                            foreach ($attachments as $attachment) {
                                                echo '<li class="flex items-center justify-between gap-2 border rounded-lg p-2 my-2">';
                                                    echo '<a href="' . BASE_URL . $attachment['file_path'] . '" class="text-blue-700 hover:underline" download="' . htmlspecialchars($attachment['file_name']) . '">' . htmlspecialchars($attachment['file_name']) . '</a>';
                                                echo '</li>';
                                            }
                                            echo '</ul>';
                                        }
                                        // Recursive call for any replies
