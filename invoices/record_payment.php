...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['invoice_id'])) {
   ...
   if ($invoice) {
            ...
            if ($amount > $remaining_balance) {
                echo "<script>alert('Payment amount cannot be greater than remaining balance. Please try again.'); window.location.href='invoices/view?id=$invoice_id';</script>";
                exit;
            }
                    ...
                    if($stmt->execute()){
                        ...
                        if($email) {
                            ...
                            if(mail($to, $subject, $message, $headers)) {
                                 echo "<script>alert('Payment recorded, and confirmation email sent!'); window.location.href='invoices/view?id=$invoice_id&success=true';</script>";
                                exit;
                            }
                         }
                            echo "<script>alert('Payment recorded!'); window.location.href='invoices/view?id=$invoice_id&success=true';</script>";
                                exit;
                    }else{
                        echo "<script>alert('Error updating payment information on the invoice.'); window.location.href='invoices/view?id=$invoice_id';</script>";
                       exit;
                    }
         }  else{
            echo "<script>alert('Error recording payment.'); window.location.href='invoices/view?id=$invoice_id';</script>";
            exit;
        }
   } else {
