...
// Fetch initial message (if it exists)
...
WHERE discussion_messages.discussion_id = :discussion_id AND parent_id IS NULL ORDER BY sent_at ASC LIMIT 1");

$stmt->bindParam(':discussion_id', $discussion_id);
$stmt->execute();
$initial_message = $stmt->fetch(PDO::FETCH_ASSOC);

// Fetch all messages for this discussion in tree order, excluding the initial one
$stmt = $conn->prepare("SELECT discussion_messages.*, users.username as user_name, employees.name as employee_name, customers.name as customer_name, customers.profile_picture as customer_profile_picture, users.profile_picture as user_profile_picture FROM discussion_messages 
LEFT JOIN users ON discussion_messages.user_id = users.id 
LEFT JOIN employees ON discussion_messages.user_id = employees.id AND discussion_messages.user_type = 'employee'
LEFT JOIN customers ON discussion_messages.user_id = customers.id AND discussion_messages.user_type = 'customer'
WHERE discussion_id = :discussion_id AND parent_id IS NOT NULL ORDER BY sent_at ASC");
                          ...
                          <?php if($participants): ?>
                             ...
                             <?php endif; ?>
                              <form method="POST" action="" class="inline-block">
                                 <button type="submit" name="toggle_status" class="bg-blue-600 text-white px-2 py-1 rounded-lg hover:bg-blue-700 transition duration-300">
                    ...
                    <div class="bg-gray-50 p-4 rounded-lg mb-4" >
                         <div class="flex items-center mb-2">
                                    ...
                                    if($initial_message['user_type'] == 'user' && $initial_message['user_profile_picture']){
                                        ...
                                        } elseif ($initial_message['user_type'] == 'customer' && $initial_message['customer_profile_picture'] ) {
                                                         ...
                                                         <p class="text-gray-500 text-sm">
                                                            <i class="fas fa-user-circle mr-1"></i>';
                                                             if($message['user_type'] == 'user') {
                                                                 echo '  <a href="profile.php" class="hover:underline">' . htmlspecialchars($message['user_name']) .'</a> ';
